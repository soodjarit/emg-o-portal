...
#   - Screenshots (2026-09-06 pass, per feedback_qa_testcase_screenshot_export):
#     TC-01..TC-10 reuse real, live-traced screenshots already captured for
#     presentations/e2e-mode2-slab.html and e2e-mode2-lot-stock.html — same
#     fixture numbers cited above, not new captures. TC-11/TC-12 (Journal
#     Items views) have NO screenshot yet — no real capture exists for that
#     specific screen on this fixture; left blank rather than faked.
#
...
CATEGORIES = [
  dict(cat_id="E1", title="E1. ขาย (Sale — Mode 2, Serial per-slab)",
    ...
    dict(id="TC-E2E-M2-01", scenario="เพิ่มบรรทัดสินค้า Material โหมด Serial ใน Sale Order",
         ...
         shot="../presentations/assets/mode2-shot-04-so-line-select-slabs-button.png"),
    dict(id="TC-E2E-M2-02", scenario="กด \"Select Slabs\" เลือกหลายแผ่นพร้อมกันตามเลข Serial",
         ...
         shot="../presentations/assets/mode2-shot-06-so-slabs-confirmed-priced.png",
         shot_note="ภาพหน้าต่างเลือกแผ่นเอง ดูได้ที่ presentations/e2e-mode2-slab.html (mode2-shot-05) — ภาพนี้คือผลลัพธ์หลังกด Confirm Selection แล้ว"),
    dict(id="TC-E2E-M2-03", scenario="ยืนยัน (Confirm) Sale Order — ราคาเฉลี่ยคำนวณจากขนาดจริงแต่ละแผ่น",
         ...
         shot="../presentations/assets/mode2-shot-07-so-confirmed.png"),
  ]),
  dict(cat_id="E2", title="E2. ขาย (Sale — Mode 2, Lot aggregate)",
    ...
    dict(id="TC-E2E-M2-04", scenario="เพิ่มบรรทัดสินค้า Material โหมด Lot ใน Sale Order",
         ...
         shot="../presentations/assets/lot2-01-so-product.png"),
    dict(id="TC-E2E-M2-05", scenario="กด \"Select Lot Stock\" ระบุจำนวนตร.ม.บางส่วนจากล็อต",
         ...
         shot="../presentations/assets/lot2-02-wizard-qty.png"),
    dict(id="TC-E2E-M2-06", scenario="ยืนยัน (Confirm) Sale Order โหมด Lot",
         ...
         shot="../presentations/assets/lot2-04-confirmed.png"),
  ]),
  dict(cat_id="E3", title="E3. จัดส่ง (Delivery — Scan Verification)",
    ...
    dict(id="TC-E2E-M2-07", scenario="พยายาม Validate ใบส่งของโดยยังไม่กรอก Scanned Serial ให้ตรง",
         ...
         shot="../presentations/assets/mode2-shot-08-delivery-scan-error.png",
         shot_note="ภาพจากเส้นทาง Serial — เส้นทาง Lot มี error หน้าตาเดียวกัน ดูได้ที่ presentations/e2e-mode2-lot-stock.html (lot2-06-scan-error)"),
    dict(id="TC-E2E-M2-08", scenario="กรอก/สแกน Scanned Serial ให้ตรงครบทุกบรรทัดแล้ว Validate",
         ...
         shot="../presentations/assets/mode2-shot-11-delivery-done.png"),
  ]),
  dict(cat_id="E4", title="E4. บัญชี (Accounting + Payment)",
    ...
    dict(id="TC-E2E-M2-09", scenario="สร้างและยืนยันใบแจ้งหนี้ลูกค้า (Customer Invoice)",
         ...
         shot="../presentations/assets/mode2-shot-13-invoice-posted.png",
         shot_note="ภาพตัวอย่าง Serial — ตัวอย่าง Lot (INV/2026/00016) ดูได้ที่ presentations/e2e-mode2-lot-stock.html (lot2-11-invoice-posted)"),
    dict(id="TC-E2E-M2-10", scenario="รับชำระเงิน (Register Payment) บนใบแจ้งหนี้",
         ...
         shot="../presentations/assets/lot2-13-paid-final.png"),
