...
<div class="layout">
  ...
  <header>
    ...
    <div class="sub">เรื่องราวเดียวต่อเนื่องกันตั้งแต่มี Block พร้อมอยู่แล้วในสต็อก ยืนยันคำสั่งขายทั้งที่ยังไม่มีแผ่นหินจริง จนถึงฝ่ายผลิตตัดแผ่นจริง (คราวนี้ก้อนเดียวตัดได้หลายแผ่นในรอบเดียว) และออกใบแจ้งหนี้ ไม่ใช่การเทสฟีเจอร์แยกส่วน และไม่มีข้อไหนเป็นการดักฟิลด์บังคับ/error ทั่วไป (ยกเว้นข้อ Confirm MO ก่อนย้าย Block และข้อ Scan Verification ซึ่งเป็น gate จริงของกระบวนการ) — เดินตามได้เองแม้ไม่ใช่ technical user เขียนใหม่ทั้งชุด 16 ก.ย. 2026 รอบกลไกตัด Block→Slab แบบวางแผน/บันทึกจริง+Yield % (ADR-066) พร้อม banner นำทาง+ปุ่ม "ย้ายด่วน" (Session 136-138) ตัวเลข "ตัวอย่างจริง" มาจากการรันจริงรอบนี้บน mbx-ee-dev ไม่ใช่ตัวเลขสมมติ</div>
    <div class="stat-row">
  ...
  <div class="flow-banner">
    <b>เรื่องราวที่ใช้ทดสอบจริง (เขียนใหม่ทั้งชุด 16 ก.ย. 2026, กลไกวางแผน/บันทึกจริง+Yield %):</b> Block BLK-26-0030 (E2E Mode3 Fresh Test, ก้อนใหม่) &rarr; SO S00131 ยืนยันที่ราคาตั้งต้น 4,815.00 (รวม VAT) &rarr; Manufacturing Order EG01/STCUT/00044 (Target 0.80&times;0.50 ม. &rarr; ระบบวางแผน 2 แผ่นอัตโนมัติ) &rarr; บล็อกครั้งแรกเพราะ Block ยังไม่ได้ย้ายไปสถานีตัด &rarr; กดปุ่ม "ย้ายด่วน (Move to Production)" ครั้งเดียวจบ &rarr; Confirm สำเร็จ &rarr; Work Orders (Transport+Gangsaw) เสร็จ &rarr; บันทึกขนาดจริงต่อแผ่น (Yield 97.78%) &rarr; Complete Cut ได้แผ่นจริง 2 แผ่นจาก 1 ก้อน (BLK-26-0030-1 จองเข้า SO, BLK-26-0030-2 เหลือเป็นสต็อกขายต่อ) &rarr; SO ราคาคำนวณใหม่อัตโนมัติเป็น 1,840.29 &rarr; Delivery EG01/OUT/00085 (สแกนยืนยัน) &rarr; Invoice INV/2026/00033 (1,840.29 รวม VAT, COGS 150.00, Posted + In Payment) — <b>เดินสดผ่านหน้าจอ Odoo จริงทุกขั้นตอนบน mbx-ee-dev</b> ไม่ใช่ผ่าน <code>odoo shell</code><br><br>
    <b>✅ ยืนยันแล้ว:</b> กลไกตัด Block→Slab แบบวางแผน/บันทึกจริงหลายแผ่น+Yield % (ADR-066) พร้อม banner นำทาง+ปุ่ม "ย้ายด่วน" ใหม่ (Session 136-138) ยังคงพฤติกรรมสำคัญทั้งหมดถูกต้อง — วางแผนจำนวนแผ่นอัตโนมัติจาก CBM คงเหลือ, auto-hold แผ่นที่ตรงความต้องการเข้า SO line ส่วนแผ่นเกินค้างเป็นสต็อกขายต่อ, ราคาคำนวณใหม่อัตโนมัติหลังตัด, บัญชีลงถูกบัญชีเฉพาะของ Mode 3, ไม่มี COGS ซ้ำจากใบส่งของ (ดู TC-E2E-M3-15) — นี่คือการทดสอบผ่านเบราว์เซอร์จริงครั้งแรกของกลไกนี้ตั้งแต่เพิ่ม banner/ปุ่มใหม่
  </div>
