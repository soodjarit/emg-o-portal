...
CATEGORIES = [
  dict(cat_id="E1", title="E1. ซื้อ (Purchase)",
    ...
    dict(id="TC-E2E-M1-02", scenario="สร้าง Block (Bundle) ผูกกับ PO — ขั้นตอนนี้คือการรับสต๊อกจริง",
         ...
         shot="assets/e2e-mode1/m1-02-bundle.png"),
  ...
  dict(cat_id="E2", title="E2. ขาย (Sale — Mode 1)",
    ...
    dict(id="TC-E2E-M1-04", scenario="สร้าง Sale Order ให้ลูกค้า",
         ...
         shot_note="ภาพจริงจากบรรทัดเดียวกันหลัง Confirm แล้ว (ปุ่ม \"Sell Whole Block\" ยังโชว์อยู่แม้ยืนยันแล้ว) — ไม่มีภาพแยกของสถานะ Quotation ก่อนกดปุ่ม เพราะขั้นตอนนี้ทำผ่านการันตีข้อมูลจริงครั้งเดียวจบ"),
    dict(id="TC-E2E-M1-05", scenario="กด \"Sell Whole Block\" เลือกก้อนที่ซื้อมา",
         ...
         shot="assets/e2e-mode1/m1-06-so-confirmed.png",
         shot_note="ภาพเดียวกับ TC-E2E-M1-06 (SO หลัง Confirm) — เห็นผลลัพธ์ของการเลือก Block นี้ในบรรทัด (Block/Lot = BLK-26-0016) แต่ไม่มีภาพแยกของหน้าต่าง \"Select Block\" ระหว่างเลือก"),
  ...
  dict(cat_id="E4", title="E4. จัดส่ง (Delivery)",
    ...
    dict(id="TC-E2E-M1-07", scenario="เปิดใบส่งของ (Delivery Order) ที่เกิดขึ้นอัตโนมัติ",
         ...
         shot_note="ภาพจริงของใบส่งของนี้ แต่เป็นสถานะ Done (หลัง Validate แล้ว) เนื่องจากทดสอบจริงเดินจนจบไม่ได้ย้อนกลับมาถ่ายสถานะ Ready อีกครั้ง — จำนวน/บรรทัดสินค้าที่เห็นตรงกับที่อธิบายไว้"),
  ...
  dict(cat_id="E5", title="E5. บัญชี (Accounting)",
    ...
    dict(id="TC-E2E-M1-12", scenario="ตรวจว่าต้นทุนขาย (COGS) ถูกบันทึกซ้ำหรือไม่ — known issue",
         ...
         shot="assets/e2e-mode1/m1-12-journal-entry-stj.png"),
