from qa_data_e2e_mode4 import CATEGORIES
...
<title>EMG-O — Test Cases: E2E Flow (Mode 4)</title>
<style>
  :root{
    --copper:#B8763E;
    --copper-dark:#8F5A2C;
...
<a id="export-btn" href="../client-docs/test-cases/emg-o-test-cases-e2e-mode4.xlsx" download="EMG-O Test Cases - E2E Mode 4.xlsx" title="ดาวน์โหลดไฟล์ Excel"><svg viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3v12"/><path d="M7 10l5 5 5-5"/><path d="M4.5 19.5h15"/></svg>Export Excel</a>
...
<div class="layout">
  ...
  <div class="flow-banner">
    <b>เรื่องราวที่ใช้ทดสอบจริง (รันใหม่ทั้งชุด 14 ก.ย. 2026):</b> BOQ0029 (ลูกค้า "ทดสอบ Mode4 Fresh - Sample Project") &rarr; Generate Quotation &rarr; SO S00126 (ติ๊ก Sold as Project) &rarr; Select Lot Stock (Vietnam Limestone Lot BLK-26-0004, 0.5 ตร.ม.) &rarr; Confirm &rarr; Project 41 + 4 Tasks สร้างอัตโนมัติ &rarr; Down Payment 3 งวด INV/2026/00028 (442.98, 30%) + 00029 (590.64, 40%) + 00030 (442.98, 30%) รวม 1,476.60 บาท &rarr; Delivery EG01/OUT/00080 (Done) &rarr; Task "Dry-Lay" Mark Done &rarr; Milestone "ปูพื้นเสร็จสมบูรณ์ (Dry-Lay Complete)" ติ๊กถูกอัตโนมัติ &rarr; Project Dashboard: Total Revenues 1,380 = Invoiced 1,380 บาท — <b>เดินสดผ่านหน้าจอ Odoo จริงทุกขั้นตอนบน mbx-ee-dev</b> ไม่ใช่ผ่าน <code>odoo shell</code><br><br>
    <b style="color:var(--green)">✅ ยืนยันแล้วว่า 2 บั๊กจริงที่เคยพบก่อนหน้ายังคงปิดอยู่จริง:</b> (1) ราคาที่ BOQ ตั้งไว้ไม่ถูกเขียนทับด้วยราคาป้ายสินค้าปกติหลังเลือกสต็อก Lot (ดู TC-03 — ยอด Materials 900 บนหน้า Dashboard คือหลักฐาน) (2) เส้นทางส่งของดึงสต็อกจากจุดที่ของจริงอยู่ (Stone Available) ไม่ใช่จุดพัก Stone Hold ที่โหมด Lot ไม่เคยย้ายของเข้าจริง (ดู TC-09) — พบเพิ่มเติมด้วยว่ากลไก BOQ→Project→Milestone→Down Payment นี้ไม่ได้ถูกกระทบจากการเปลี่ยนกลไกตัดหินเมื่อ 13 ก.ย. 2026 เลย (คนละกลไกกัน)
  </div>
...
if __name__ == '__main__':
    with open('../test-cases-e2e-mode4.html', 'w', encoding='utf-8') as f:
        f.write(PAGE)
